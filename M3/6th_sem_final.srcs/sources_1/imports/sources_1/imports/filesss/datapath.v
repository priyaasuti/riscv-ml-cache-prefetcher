...
//  datapath.v — FIXED v3
//
//  KEY FIX: cache_pf_busy = pf_inflight_w ONLY
//  (not cache_stall | pf_inflight_w as before)
//
//  With the new l1_cache, pf_inflight_w is only HIGH during
//  S_PF_MISS and S_PF_FILL states. Demand fills (S_MISS/S_FILL)
//  do NOT set pf_inflight. This means the controller correctly
//  sees cache_busy=0 even during CPU demand stalls, and forwards
//  new pf_req immediately.
//
//  The l1_cache's internal pending_pf register does the real
//  queuing — it latches any pf_req immediately regardless of
//  cache state, and serves it on the first idle cycle.
// ============================================================
...
// ── FIX: cache_pf_busy = pf_inflight ONLY ─────────────────
// Previously: cache_pf_busy = cache_stall | pf_inflight_w
// That made cache_busy=1 during all demand misses, so the
// controller never forwarded pf_req to the cache.
// Now: only block when a prefetch fill is actually in-flight.
wire cache_pf_busy = pf_inflight_w;
...
prefetch_controller PREF_CTRL (
    ...
    .cache_busy(cache_pf_busy),   // = pf_inflight_w ONLY (not cache_stall)
